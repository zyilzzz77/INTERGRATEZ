...
export default function BotTeleMiniApp({ lang }: { lang: string }) {
  ...
  const rawUserId = searchParams.get("user_id") || searchParams.get("id");
  const rawFullName = searchParams.get("name") || searchParams.get("full_name");
  const rawUsername = searchParams.get("username");

  const userId = tgUser?.id || rawUserId || "-";
  const fullName = tgUser 
    ? `${tgUser.first_name || ""} ${tgUser.last_name || ""}`.trim() 
    : (rawFullName || "Pengguna Telegram");
  const username = tgUser?.username ? `@${tgUser.username}` : (rawUsername ? `@${rawUsername}` : "-");
        ...
        <div className="grid gap-6 md:grid-cols-3">
          ...
          <div className="rounded-2xl border-[3px] border-black bg-white p-4 shadow-neo-sm">
            ...
            <ul className="mt-3 space-y-2 text-sm text-neutral-700 list-disc list-inside">
              ...
              <li>Sistem akan otomatis mendeteksi identitas akun Telegram.</li>
