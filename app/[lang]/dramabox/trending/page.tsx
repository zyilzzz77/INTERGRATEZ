...
export default function TrendingPage() {
    ...
    useEffect(() => {
        async function fetchData() {
            ...
            try {
                const res = await fetch("/api/dramabox/trending", { cache: "no-store" });
            ...
            <div className="mx-auto max-w-6xl px-4 py-10">
                ...
                <div className="mb-8">
                    ...
                    <h1 className="mt-4 text-3xl font-black text-black sm:text-4xl flex items-center gap-3">
                        📈 <span className="px-2 rounded-lg border-[3px] border-black bg-gradient-to-r from-[#ffec99] to-[#ffd1a1]">Trending</span>
                    </h1>
                    <p className="mt-2 text-sm font-bold text-black/70">Drama paling populer saat ini</p>
                </div>
                            ...
                            {data.map((item, index) => (
                                ...
                                >
                                    <div className="relative aspect-[2/3] w-full overflow-hidden bg-[#f3f3f3]">
                                        ...
                                        {/* Rank Number */}
                                        {item.rankVo && (
                                            <div className="absolute top-2 left-2 z-20 flex items-center justify-center h-7 w-7 rounded-lg bg-black text-white text-xs font-black shadow-neo-sm border-2 border-white">
                                                {item.rankVo.sort}
                                            </div>
                                        )}

                                        {item.corner && (
                                            <div className="absolute top-2 right-2 z-20 rounded-md px-2 py-0.5 text-[10px] font-black text-black bg-white border-[2px] border-black shadow-neo-sm" style={{ backgroundColor: item.corner.color || "#ffd1a1" }}>
                                                {item.corner.name}
                                            </div>
                                        )}
                                        ...
                                        <div className="absolute bottom-0 left-0 right-0 p-3 z-20">
                                            <div className="flex items-center gap-2 text-[10px] font-black text-white flex-wrap">
                                                {item.rankVo?.hotCode && (
                                                    <span className="flex items-center gap-1 bg-black/60 px-1.5 py-0.5 rounded-full">🔥 {item.rankVo.hotCode}</span>
                                                )}
                                    ...
                                    <div className="flex flex-1 flex-col p-3">
                                        ...
                                        <div className="mt-2 flex flex-wrap gap-1">
                                            {item.tags && item.tags.length > 0 ? (
                                                ...
                                                <span className="text-[10px] text-black/70 line-clamp-1 font-bold">{item.protagonist}</span>
                                            ) : null}
                                        </div>
                                    </div>
                                </Link>
