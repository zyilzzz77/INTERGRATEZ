...
export default function ForYouPage() {
    ...
    useEffect(() => {
        async function fetchData() {
            try {
                const res = await fetch("/api/dramabox/foryou");
            ...
            <div className="mx-auto max-w-6xl px-4 py-10">
                ...
                <div className="mb-8">
                    ...
                    <h1 className="text-3xl font-black text-white sm:text-4xl flex items-center gap-3">
                        ✨ <span className="text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-pink-500">Untukmu</span>
                    </h1>
                    <p className="mt-2 text-sm text-neutral-400">Drama pilihan khusus untukmu</p>
                </div>
                            ...
                            {data.map((item, index) => (
                                ...
                                >
                                    <div className="relative aspect-[2/3] w-full overflow-hidden bg-neutral-800">
                                        ...
                                        {item.corner && (
                                            <div className="absolute top-2 left-2 z-20 rounded-md px-2 py-0.5 text-[10px] font-bold text-white shadow-sm backdrop-blur-sm" style={{ backgroundColor: item.corner.color || "#F54E96" }}>
                                                {item.corner.name}
                                            </div>
                                        )}
                                        {item.rankVo && (
