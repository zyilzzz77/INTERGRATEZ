        ...
        async function fetchInitialData() {
            try {
                ...
                if (json.status && Array.isArray(json.data)) {
                    ...
                    setNextPage(json.nextPage ?? 2);
                }
            } catch (error) {
                console.error("Failed to fetch Netshort data:", error);
            } finally {
                setLoading(false);
            }
        }
    ...
    const loadMore = async () => {
        ...
        try {
            ...
            if (json.status && Array.isArray(json.data) && json.data.length > 0) {
                ...
                const newItems = json.data.filter((item: NetshortItem) => !existingIds.has(item.id));
                ...
                setHasMore(json.hasMore ?? false);
                setNextPage(json.nextPage ?? nextPage + 1);
            } else {
                setHasMore(false);
            }
        } catch (error) {
        ...
        <LazyMotion features={domAnimation}>
            ...
            <div className="mx-auto max-w-6xl px-4 py-10">
                ...
                <div className="mb-10 text-center">
                    <div className="mb-4 inline-flex h-20 w-20 items-center justify-center rounded-3xl bg-gradient-to-br from-cyan-500 to-teal-600 shadow-lg ring-1 ring-white/20 text-4xl">
                        🎬
                    </div>
                    <h1 className="text-3xl font-black text-white sm:text-4xl">
                        Netshort <span className="text-cyan-500">Drama</span>
                    </h1>
                    ...
                    <form onSubmit={handleSearch} className="mx-auto mt-8 max-w-xl">
                        <div className="relative">
                            <input
                                ...
                                className="w-full rounded-full border border-white/10 bg-white/5 px-6 py-3 pl-12 text-white placeholder-neutral-500 backdrop-blur-sm transition-all focus:border-cyan-500/50 focus:bg-white/10 focus:outline-none focus:ring-4 focus:ring-cyan-500/10"
                            />
                            ...
                            <button
                                type="submit"
                                className="absolute right-2 top-1/2 -translate-y-1/2 rounded-full bg-cyan-600 px-4 py-1.5 text-sm font-bold text-white transition-colors hover:bg-cyan-500"
                            >
                                Cari
                            </button>
                        </div>
                    ...
                    >
                        ...
                        {displayData.length > 0 && (
                            <h2 className="text-xl font-bold text-white sm:text-2xl flex items-center gap-2">
                                {isShowingSearch ? (
                                    <>🔍 Hasil Pencarian: <span className="text-cyan-400">&quot;{searchQuery}&quot;</span></>
                                ) : (
                                    <>🔥 Semua Serial</>
                                )}
                            </h2>
                        ...
                        <div className="grid gap-3 grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
                            {displayData.map((item: any, index: any) => (
                                <Link
                                    href={`/netshort/detail?id=${item.id}`}
                                    key={`${item.id}-${index}`}
                                    className="group relative flex flex-col overflow-hidden rounded-xl border border-white/5 bg-white/5 transition-all duration-300 hover:-translate-y-1 hover:border-cyan-500/30 hover:shadow-lg hover:shadow-cyan-500/10"
                                >
                                    {/* Cover Image */}
                                    <div className="relative aspect-[3/4] w-full overflow-hidden bg-neutral-800">
                                        ...
                                        {/* New Badge */}
                                        {item.isNew && (
                                            <div className="absolute top-2 left-2 z-20 rounded-md bg-cyan-600/90 px-2 py-0.5 text-[10px] font-bold text-white shadow-sm backdrop-blur-sm">
                                                ✨ Baru
                                            </div>
                                        )}
                                        ...
                                        {/* Heat Score */}
                                        {item.heatScore && (
                                            <div className="absolute top-2 right-2 z-20 rounded-md bg-orange-600/90 px-2 py-0.5 text-[10px] font-bold text-white shadow-sm backdrop-blur-sm">
                                                🔥 {item.heatScore}
                                            </div>
                                        )}
                                    ...
                                    <div className="flex flex-1 flex-col p-3">
                                        <h3 className="line-clamp-2 text-sm font-bold text-white group-hover:text-cyan-400 leading-tight">
                                            {item.title}
                                        </h3>
