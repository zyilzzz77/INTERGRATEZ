...
function YouTubeWatchContent({ dict, lang }: { dict: any; lang: string }) {
    ...
    const handleRelatedWatch = (url: string) => {
        ...
        router.push(`/${lang}/search/youtube/watch?url=${encodeURIComponent(url)}&q=${encodeURIComponent(queryParam || "")}`);
    };
    ...
    return (
        <div className="min-h-screen bg-neutral-900 px-4 py-8">
            <div className="mx-auto max-w-[1600px]">
                {/* Back Button */}
                <button
                    onClick={() => router.push(`/${lang}/search/youtube`)}
                        ...
                        <div className="overflow-hidden rounded-2xl bg-black shadow-2xl ring-1 ring-white/10">
                            {loading ? (
                                <div className="flex aspect-video w-full flex-col items-center justify-center bg-white/5 text-white">
                                    ...
                                    <p className="mt-4 animate-pulse font-medium">{dict.watchLoading}</p>
                                </div>
                            ) : error ? (
                                <div className="flex aspect-video w-full flex-col items-center justify-center bg-white/5 p-8 text-center text-white">
                                    ...
                                    <div className="mt-6">
                                        <p className="text-xl font-bold text-red-400">{dict.watchErrorTitle}</p>
                            ...
                            {relatedLoading ? (
                                ...
                                <div className="rounded-xl border border-dashed border-neutral-700 p-8 text-center">
                                    <p className="text-sm text-neutral-500">
                                        {queryParam ? dict.watchNoRelated : dict.watchSearchForPlaylist}
                                    </p>
                                </div>
                            )}
                        </div>
...
export default function YouTubeWatchClient({ dict, lang }: { dict: any; lang: string }) {
    return (
        <Suspense fallback={
            <div className="flex min-h-screen items-center justify-center bg-neutral-900 text-white">
                <div className="flex flex-col items-center gap-4">
                    <div className="h-12 w-12 animate-spin rounded-full border-4 border-red-500 border-t-transparent shadow-lg shadow-red-500/20" />
                    <p className="animate-pulse font-medium">{dict.watchLoadingPage}</p>
                </div>
            </div>
        }>
            <YouTubeWatchContent dict={dict} lang={lang} />
        </Suspense>
    );
}
