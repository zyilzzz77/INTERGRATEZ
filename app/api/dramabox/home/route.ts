    ...
    try {
        ...
        for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
            try {
                ...
                // Map the new DramaBox API response to the frontend format
                const records = raw?.recommendList?.records || [];
                const sectionTitle = raw?.recommendListTitle || "Rekomendasi Populer";
                ...
                if ((isTimeout || isNetwork) && attempt < MAX_RETRIES) {
                    console.log(`[DramaBox Home] Attempt ${attempt + 1} failed (${err.code || 'timeout'}), retrying...`);
                    continue;
                }
                break;
            }
